...
exec > >(tee -a "$log") 2>&1
echo "=== 30-nested $(date -u +%Y-%m-%dT%H:%M:%SZ) VARIANT=${VARIANT:-default} ==="
...
if "$here/run-podman.sh" /usr/local/bin/outer.sh /usr/local/bin/inner-tool-call.sh; then
  ...
  echo "FAIL nested bwrap battery (VARIANT=${VARIANT:-default}); rerun with VARIANT=unmask, then label-disable,unmask, then no-seccomp, then keep-ns, and record each"
fi
echo "=== end 30-nested ==="
