...
- [x] One default agent (four base tools + `run_script` + Python REPL, and since P1.9 `ask_user`, D17) shipped in-repo (`profiles/`; `tests/shipped.rs` resolves it end to end)

### P1.9 — Protocol server and first client — `in progress` 🧑 (server, daemon, forwarder and launcher landed with tests on 2026-09-06; the one open box is the live Zed session, which needs a human at an editor)

Open question §15.2 (wire protocol) is decided here. Spec: `docs/specs/protocol.md`; client setup: `docs/clients/zed.md`. Served protocol version is ACP **v1**: the SDK (`agent-client-protocol` 2.1.0) keeps v2 behind `unstable_protocol_v2` and the official clients negotiate v1, the case ADR-0004 resolution 1 anticipated; v2 is a follow-on.
...
- [x] JSON-RPC over stdio (kernel) and unix socket (supervisor daemon that spawns one kernel process per session, D4): `grist-kernel` (`crates/orchestrator/src/bin/grist_kernel.rs`, one session, refuses a second) and `grist-daemon` (`src/acp/daemon.rs`: spawns `grist-kernel` per session, forwards every message typed and unchanged); `tests/daemon.rs` drives the real binaries
- [x] Socket auth: file permissions plus peer-credential check (D11): socket mode `0600` (asserted in `tests/daemon.rs`), directory `0700` when the daemon creates it, `peer_cred().uid()` must equal the daemon's uid (the rejection path is not testable in a one-uid test run)
- [x] Protocol covers: start session, send user message, stream events, `ask_user` round trip (D17), cancel (D15), suspend, resume, replay, end session (`tests/acp_agent.rs`: `session/new`, `session/prompt` with tool-call/text updates, `elicitation/create` ↔ `ask_user`/`user_answer` seen through `_grist/subscribe`, `session/cancel` → `stopReason: cancelled` + `cancelled` in the log, `run_script` suspend → process-exit waker → resume inside one prompt with `suspended`/`task_update` in the log, `session/load` replaying the history and continuing, `session/close` → `session_ended`). Suspend/resume in P1 is kernel-driven (a task wait) and cross-process resume is `session/load`; an operator-triggered suspend has no protocol surface until external wakers exist (P3.4)
- [x] Remote use documented as an SSH-forwarded unix socket; no websocket in this phase (D11): `docs/clients/zed.md` §2 (`ssh -L` to a local socket, or to a TCP port for Windows via `grist-connect --tcp`)
- [ ] 🧑 First client: **Zed** as an ACP client over `grist-connect` (ADR-0004 §2; JetBrains second known-good), configured by a checked-in `agent_servers` snippet (`docs/clients/zed-agent-servers.json`, done); runs on Windows, macOS, and Linux (D14). No CLI (§13, "CLI-less"); the Tauri shell moves to P3. **Open:** the live session from Zed against the daemon's socket (the ADR-0004 exit demonstration) needs a human at an editor; every other step of that demonstration is asserted by `tests/acp_agent.rs` and `tests/daemon.rs` with an SDK client in Zed's place
- [x] Every protocol message maps to or from an `Event`; no protocol-only state: `src/acp/project.rs` is a pure `Event`/`ModelDelta` → `session/update` function, `_grist/event` carries the redacted `Event` verbatim, and the daemon holds only the session-id → process map (`docs/specs/protocol.md` §4)
- [x] Launcher: `profiles::KernelInputs` → `KernelConfig` + `SessionInit` (`src/launcher.rs`; the reference shape in `tests/launcher.rs` now delegates its registry to it), backend selection by `sandbox_backend` (`none` only in a `dev-sandbox-none` build, else `LaunchError::Backend`), endpoint URL via `host::endpoint_url_from_env`, provider from `Quirks` (`tests/daemon.rs` runs the real client against a fake SSE endpoint), model-profile `tool_descriptions` applied by wrapping the compiled tools' definitions (`Described`; asserted on the `ModelRequest` in `tests/acp_agent.rs`)
